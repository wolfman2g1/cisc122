...
public class TwoDimArray {
	...
	public TwoDimArray(double[][] array) {
		...
		for (int row = 0; row < array.length; row++) {
			for (int col = 0; col < array[row].length; col++) {
				twoDimArray[row][col] = array[row][col]; // this should assign the index at test to the index of the 2d
															// array
			}
		}
	...
	public double getColumnTotal(int d) {
		...
		for (int row = 0; row < twoDimArray.length; row++) {
			for (int col = 0; col < twoDimArray[row].length; col++) {
				colTotal += twoDimArray[row][d]; // loop through the row but only look at the subscript for the column
	...
	/**
	 * this method calculated the highest value in the array Starts at the beginng
	 * of the array and searches for the largest value
	 *
	 * @return max_value the highest value
	 */
	public double getArrayMaxValue() {
		final int ROW_SIZE = twoDimArray.length;
		double max_value = twoDimArray[0][0];
		for (int i = 0; i < ROW_SIZE; i++) {
	...
	/**
	 * this method calculated the lowest value in the array
	 *
	 * @return min_value the lowest value in the array
	 */
	public double getArrayMinValue() {
		final int ROW_SIZE = twoDimArray.length;
		double min_Value = twoDimArray[0][0]; // start at the beginning
		for (int i = 0; i < ROW_SIZE; i++) {
			final int COL_SIZE = twoDimArray[i].length;
			for (int j = 0; j < COL_SIZE; j++) {
